...
class knn:
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        if self.x_train is None or self.y_train is None:
            raise ValueError("Model not fitted yet. fit() function call needed")
        ...
        for i in range(n):
            ...
            knn_idx = self.get_k_nearest_neighbors(distances)
            knn_labels = self.y_train[knn_idx]
            predictions[i] = self.most_common_label(knn_labels)
    ...
    def predict_proba(self, X):
        ...
        for i in range(n):
            ...
            p1 = np.mean(self.y_train[knn_idx])
            proba[i, 1] = p1 
            proba[i, 0] = 1- p1
...
def evaluate_classification_metrics(y_true, y_pred, positive_label):
    ...
    # Accuracy
    accuracy = (tp + tn)/(tp + tn + fp + fn) if (tp +fp + tn + fn) != 0 else 0.0
...
def plot_calibration_curve(y_true, y_probs, positive_label, n_bins=10):
    ...
    for i in range(n_bins):
        left = bin_edges[i]
        right = bin_edges[i + 1]
        mask = (y_probs >= left) & (y_probs < right)

        probs_in_bin = y_probs[mask]
        true_in_bin = y_true[mask]

        proportion = np.mean(true_in_bin)
...
def plot_probability_histograms(y_true, y_probs, positive_label, n_bins=10):
    ...
    plt.hist(probs_pos, bins=n_bins, alpha=0.6, label="Positive class")
    plt.hist(probs_neg, bins=n_bins, alpha=0.6, label="Negative class")

    plt.xlabel("Predicted probability for positive class")
    plt.ylabel("Count")
    plt.title("Probability Histograms by True Class")
    plt.show()
...
def plot_roc_curve(y_true, y_probs, positive_label):
    ...
    """
    # Positive labels not used cause y_true already dummyficated
    ...
    for tau in thresholds:
        y_pred_bin = (y_probs >= tau).astype(int)
        ...
        tpr_ = tp/(tp + fn) if (tp + fn) > 0 else 0.0
